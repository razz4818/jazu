...
router.post('/', extractTenantScope, async (req: Request, res: Response) => {
  try {
    ...
    const requirements: EquipmentRequirement[] = reqRows.map(r => ({
      id: r.id,
    ...
    const outcome = evaluatePpeVerification(detection as DetectionResult, requirements);

    const now = new Date().toISOString();
    const attemptId = `att_${uuidv4().substring(0, 10)}`;
...
router.get('/stats', extractTenantScope, async (req: Request, res: Response) => {
  try {
    const restaurantId = req.restaurantId!;
    ...
    const todayStats = await queryOne<{ total: number; passed: number; failed: number }>(`
      SELECT
        COUNT(*) as total,
    ...
    const workerStats = await queryOne<{ active_count: number; total_count: number }>(`
      SELECT
    ...
    const cameraStats = await queryOne<{ online_count: number; total_count: number }>(`
      SELECT
...
router.get('/history', extractTenantScope, async (req: Request, res: Response) => {
  try {
    const restaurantId = req.restaurantId!;
...
router.get('/attempt/:id', extractTenantScope, async (req: Request, res: Response) => {
  try {
    const restaurantId = req.restaurantId!;
    const { id } = req.params;
