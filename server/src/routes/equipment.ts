...
router.post('/', extractTenantScope, async (req: Request, res: Response) => {
  try {
    ...
    const existing = await queryOne('SELECT id FROM equipment_requirements WHERE restaurant_id = ? AND code = ?', [restaurantId, code]);
    ...
    const id = `eq_${uuidv4().substring(0, 8)}`;
...
router.put('/:id', extractTenantScope, async (req: Request, res: Response) => {
  try {
    ...
    const existing = await queryOne('SELECT id FROM equipment_requirements WHERE restaurant_id = ? AND id = ?', [restaurantId, id]);
    if (!existing) {
...
router.delete('/:id', extractTenantScope, async (req: Request, res: Response) => {
  try {
    ...
    const existing = await queryOne('SELECT id, is_custom FROM equipment_requirements WHERE restaurant_id = ? AND id = ?', [restaurantId, id]);
    if (!existing) {
      res.status(404).json({ error: 'Equipment requirement not found' });
      return;
    }

    await execute('DELETE FROM equipment_requirements WHERE restaurant_id = ? AND id = ?', [restaurantId, id]);
