...
import { getDb, execute, queryOne, saveDb } from './index';
...
export async function seedDatabase() {
  ...
  const existingRest = await queryOne('SELECT id FROM restaurants WHERE id = ?', [restaurantId]);
  ...
  const existingSettings = await queryOne('SELECT id FROM restaurant_settings WHERE restaurant_id = ?', [restaurantId]);
  if (!existingSettings) {
    await execute(`
  ...
  for (const eq of defaultEquipments) {
    const existing = await queryOne('SELECT id FROM equipment_requirements WHERE restaurant_id = ? AND code = ?', [restaurantId, eq.code]);
    if (!existing) {
      await execute(`
  ...
  for (const w of demoWorkers) {
    const existing = await queryOne('SELECT id FROM workers WHERE restaurant_id = ? AND worker_code = ?', [restaurantId, w.code]);
    if (!existing) {
      await execute(`
  ...
  for (const cam of demoCameras) {
    const existing = await queryOne('SELECT id FROM cameras WHERE restaurant_id = ? AND camera_code = ?', [restaurantId, cam.code]);
    if (!existing) {
      await execute(`
  ...
  const existingAttemptsCount = await queryOne<{ count: number }>('SELECT COUNT(*) as count FROM verification_attempts WHERE restaurant_id = ?', [restaurantId]);
  ...
  if (existingAttemptsCount && existingAttemptsCount.count === 0) {
    ...
    for (const sample of sampleAttempts) {
      ...
      `, [
        ...
        JSON.stringify([]),
        JSON.stringify(Object.keys(sample.snapshot).filter(k => (sample.snapshot as any)[k].detected)),
