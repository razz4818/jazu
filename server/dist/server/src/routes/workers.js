...
router.get('/:id', auth_1.extractTenantScope, async (req, res) => {
    try {
        const restaurantId = req.restaurantId;
        const { id } = req.params;
    ...
    `, [restaurantId, id, id]);
        ...
        const historyRows = await (0, db_1.queryAll)(`
...
router.post('/', auth_1.extractTenantScope, async (req, res) => {
    try {
        ...
        const existing = await (0, db_1.queryOne)('SELECT id FROM workers WHERE restaurant_id = ? AND worker_code = ?', [restaurantId, cleanCode]);
        if (existing) {
            res.status(400).json({ error: `Worker with code ${cleanCode} already exists in this restaurant` });
            return;
        }
        const id = `w_${(0, uuid_1.v4)().substring(0, 8)}`;
...
router.put('/:id', auth_1.extractTenantScope, async (req, res) => {
    try {
        ...
        const existing = await (0, db_1.queryOne)('SELECT id FROM workers WHERE restaurant_id = ? AND id = ?', [restaurantId, id]);
        if (!existing) {
            res.status(404).json({ error: 'Worker not found' });
            return;
        }
...
router.delete('/:id', auth_1.extractTenantScope, async (req, res) => {
    try {
        ...
        await (0, db_1.execute)('DELETE FROM workers WHERE restaurant_id = ? AND id = ?', [restaurantId, id]);
